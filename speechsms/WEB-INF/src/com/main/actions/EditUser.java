			...
			throws ServletException, IOException
	{
		...
		if(sub.trim().equals("Edit"))
		{
			...
			try
			{
				...
				if(val==1 )
				{
					 ...
					 rd=req.getRequestDispatcher("user_details.jsp?no=1");
			         rd.forward(req,resp);
				}
				else
				{
					rd=req.getRequestDispatcher("edituser.jsp?no=1");
			        rd.forward(req,resp);
		...
		if(sub.trim().equals("Delete"))
		{
			...
			if(chk1==null)
			{
				...
				rd=req.getRequestDispatcher("user_details.jsp?no=2");
				rd.forward(req,resp);
			}
			else
			{
				for(int i=0;i<chk1.length;i++)
				{
					...
					admindao.deleteUserfrompassword(chk1[i]);
				}
